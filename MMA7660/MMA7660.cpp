    ...
    if (!active) {
        ...
        ThisThread::sleep_for(  
            std::chrono::milliseconds(
                12 + static_cast<int>(1000.0/samplerate))); //Wait until new sample is ready, my experience is that 1/samplerate isnt needed, but datasheet says so
    }
...
float MMA7660::getSingle( int number )
{
    ...
    if (!active) {
        ...
        ThisThread::sleep_for(
            std::chrono::milliseconds(
                12 + static_cast<int>(1000/samplerate))); //Wait until new sample is ready
    }
